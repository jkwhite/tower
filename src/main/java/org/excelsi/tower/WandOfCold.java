...
public class WandOfCold extends Wand implements Createable {
    public Item[] getIngredients() {
        Item[] ing = new Item[]{new SteelTube(), new Battery(), new Nozzle(), new Potion(new Cold()), new Wire(), new RollOfTape()};
        return ing;
    }
    ...
    public void invoke(final NHBot b) {
        if(discharge(b)) {
            ...
            final int temp = t;
            b.getEnvironment().project(chosen, new BoltAttack(new Cold(temp), b, new Source("the ice bolt")));
        }
    }
