...
public class TestConcussion extends junit.framework.TestCase {
    public void testCursed() {
        Concussion s = new Concussion();
        s.setStatus(Status.cursed);
        Patsy p = new Patsy();
        p.setMaxHp(1);
        s.inflict(p);
        assertTrue("did not get concussion boost", p.getMaxHp()>1);
        assertTrue("did not get overdose", p.getAfflictions().get(0) instanceof Overdose);
        int tries = 0;
        while(p.isAfflictedBy("concussion")) {
            p.tick();
            if(++tries==1000) {
    ...
    public void testUncursed() {
        ...
        Patsy p = new Patsy();
        s.inflict(p);
        assertTrue("did not get concussion boost", p.getMaxHp()>0);
    }
